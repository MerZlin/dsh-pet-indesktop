...
@dataclass(frozen=True)
class LifecycleEvent(SemanticEvent):
    pass
...
@dataclass(frozen=True)
class ActionEvent(ToolCallEvent):
    pass
...
@dataclass(frozen=True)
class ApprovalEvent(SemanticEvent):
    status: str = ""


@dataclass(frozen=True)
class QuestionEvent(SemanticEvent):
    status: str = ""


@dataclass(frozen=True)
class UserActionEvent(SemanticEvent):
    action: str = ""
...
def normalize_event(record: AgentEvent | dict, *, source_hint: str = "", agent_name_hint: str = "") -> SemanticEvent | None:
    ...
    typ = ev.event
    lower = typ.lower()
    common = dict(source=ev.source, agent_name=ev.agent_name, session_id=ev.session_id, step=ev.step, data=data, event=_EVENT_ALIASES.get(lower, lower))
    ...
    if lower in {"assistant/message", "assistant/chunk", "agent_reasoning", "agent_reasoning_raw_content", "reasoning"}:
        return ReasoningEvent(
            **common, summary=str(data.get("summary") or data.get("text") or data.get("content") or "")[:300], delta=lower.endswith("chunk") or "delta" in data
        )
    if lower in {"tool/call", "command/run", "tool-workflow/run-start", "exec_command_begin", "mcp_tool_call_begin"}:
        ...
        cls = (
            "ACTION"
            if tool.lower() in _ACTION or lower == "command/run"
            else "EXPLORATION"
            if tool.lower() in _EXPLORATION or lower in _EXPLORATION
            else "OTHER"
        )
    ...
    if lower in {"tool/result", "exec_command_end", "mcp_tool_call_end"}:
        ...
        result = ToolResultEvent(
            **common, tool=str(data.get("tool") or data.get("toolName") or ""), ok=ok, target=str(data.get("target") or data.get("path") or "")[:300]
        )
    ...
    if lower == "llm/retry":
        ...
        return RetryEvent(
            **common,
            code=str(failure.get("code") or data.get("errorCode") or ""),
            message=str(failure.get("message") or data.get("errorMessage") or "")[:300],
    ...
    if lower.startswith("approval/"):
        return ApprovalEvent(**common, status=lower.split("/", 1)[1])
    if lower.startswith("question/"):
        return QuestionEvent(**common, status=lower.split("/", 1)[1])
    if lower == "interaction/resolved":
        return InteractionResolvedEvent(
            **common,
            kind=str(data.get("kind") or ""),
            request_id=str(data.get("requestId") or ev.request_id or ""),
        ...
        if "resolved" in action or "decided" in action:
            ...
            return InteractionResolvedEvent(
                **common,
                ...
                approval_id=str(data.get("approvalId") or ""),
                call_id=str(data.get("callId") or ""),
                outcome=str(data.get("outcome") or data.get("decision") or ""),
            )
        return UserActionEvent(**common, action=action)
    if lower in {"control-result", "bridge/control-result", "watchdog/control-result"}:
        return ControlResultEvent(
            **common,
            request_id=ev.request_id or str(data.get("requestId") or ""),
