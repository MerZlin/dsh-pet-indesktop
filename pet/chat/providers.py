...
def test_connection(config, timeout: float = 10.0):
    ...
    try:
        ...
        if exc.code in (401, 403):
            return False, f"认证失败（HTTP {exc.code}）：{msg}"
...
class SSEParser:
    ...
    def feed(self, chunk: bytes):
        ...
        if len(self._buffer) > 1024 * 1024:
            raise ProviderError("SSE 缓冲超过 1MB 上限（服务端未按事件分隔）")  # 审查 GLM-M5：防异常 provider 无界堆积
        out = []
        while True:
            found = None
            for marker in (b"\r\n\r\n", b"\n\n"):
            ...
            self._buffer = self._buffer[i + len(marker) :]
            data = []
            for line in event.replace(b"\r\n", b"\n").split(b"\n"):
                if line and not line.startswith(b":") and line.startswith(b"data:"):
                    data.append(line[5:].lstrip().decode("utf-8", "replace"))
            if not data:
                continue
            payload_text = "\n".join(data).strip()
            if not payload_text:
                continue  # 空 data: 心跳行（部分 OpenAI 兼容服务用作 keepalive），跳过不解析——否则 json.loads('') 中止整条流（审查 DS-L20）
            ...
            try:
                ...
                raise ProviderError("Provider 返回了无效 JSON") from exc
            if isinstance(payload, dict) and payload.get("error"):
                err = payload["error"]
                msg = err.get("message", "Provider 返回错误") if isinstance(err, dict) else str(err)
                raise ProviderError(str(msg))
...
class OpenAICompatibleProvider:
    def stream(
        self, messages: list[dict[str, Any]], config: ProviderConfig, cancel_event: threading.Event, response_holder: list | None = None
    ) -> Iterator[str]:
        endpoint = normalize_chat_endpoint(config.base_url, config.chat_path)
        ...
        req = urllib.request.Request(endpoint, data=json.dumps(payload, ensure_ascii=False).encode("utf-8"), headers=headers, method="POST")
        try:
            response = urllib.request.urlopen(req, timeout=config.timeout, context=_make_ssl_context(config.verify_ssl))
        except urllib.error.HTTPError as exc:
            detail = exc.read(2048).decode("utf-8", "replace")
            raise ProviderError(_safe_error_detail(detail), exc.code) from exc
        except urllib.error.URLError as exc:
            reason = exc.reason
            hint = _CERT_HINT if _is_cert_verify_error(reason) else ""
