...
> **范围**：341 个已跟踪文件，累计 `+12113 / -9725`；本报告及其索引登记在生成后作为证据文档新增。
...
本节的逐文件增删数字由以下命令在工作树中生成：

```powershell
& 'D:\DELL\Git\cmd\git.exe' diff --numstat 9d294ea
```

由于本报告自身在生成时尚未加入 Git，下面的机器生成清单覆盖基线到当前工作树的已跟踪变更；报告文件自身在新增文件清单中单列。增删数字不是行为覆盖率数字。
...
| `LOG.md` | 32 | 0 | 记录已完成的工具链、门禁、覆盖率合并和未完成的跨平台实机验证。 |
| `README.md` | 20 | 1 | 补齐新人质量入口、Python 支持范围和完整/快速/矩阵检查命令。 |
