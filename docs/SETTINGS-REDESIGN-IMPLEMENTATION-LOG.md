...
- 菜单编排与快捷启动的命令下拉统一复用 `ModernSelect` 弹层的圆角、间距、选中态和深浅色 token，不再混用原生 `QMenu` 外观。
- 别名在编排树中显示为“别名（原名）”以保留编辑上下文，实时预览和运行时菜单则遵循展示语义，仅显示别名。
- 图标覆盖新增本地文件选择，限制为可解码的 PNG/JPG/WebP/BMP/GIF/TIFF 且不超过 5 MB，并提供“完整显示 / 裁切填满”；无效或丢失文件在运行时安全回退到动作默认图标。
- macOS 安装原生 Dock 快捷菜单，提供显示桌宠、桌宠设置、AI 对话（可用时）和退出，使鼠标穿透后仍有稳定的设置恢复入口。
- 自言自语和彩蛋图片目录增加默认折叠的自适应缩略图流。它使用固定格子、行优先顺序而非破坏阅读顺序的 masonry；名称单行省略并保留完整 tooltip，图片仅在展开时解码。
...
本轮下拉统一、文件图标、Dock 恢复入口和图片目录预览：

```text
专项 184 passed（菜单 57 + 请求回归 40 + 桌宠功能 87）
分段全量 778 passed, 7 skipped（586 + 6 + 2 + 57 + 87 + 40）
```

本轮菜单编排、页内导航与图片浏览器二次改进：

- 菜单编排操作栏前置，编辑/预览面板升级为独立卡片；三列表头全部允许拖动调宽，紧凑布局仍隐藏冗余“位置”列。
- `SettingsTabContainer` 采用目标插件设置页的紧凑 toolbar segmented navigation：左对齐、内容定宽，不再铺满整个内容区。
- 菜单模式选择器、菜单编排操作菜单和快捷启动操作菜单合并为同一个 `SettingsPopup` 表面与主题刷新入口，避免样式复制后继续漂移。
- 图片目录预览改为点击“预览”后打开右侧抽屉；三列布局按当前最短列放置下一张图片，保留可读宽高比和省略文件名，不再用等高 `QListWidget.IconMode` 模拟瀑布流。

```text
菜单/桌宠功能/请求回归专项 187 passed
```

菜单弹层与编排响应式布局复核：

- 共享颜色 token 仍不足以保证一致：选择器原先使用自绘 `QWidgetAction`，操作按钮使用 `QPushButton.setMenu()`，两条路径的勾选、禁用色、原生菜单指示器、宽度与锚点均不同。
- 新增深模块 `SettingsPopupMenu / SettingsMenuButton`。调用方只声明 action；模块集中负责统一 QAction 行、右侧勾选、4px 弹层间距、至少等于触发按钮的宽度、自绘 chevron 和焦点状态。
- 菜单编排从宽/窄二态改为 wide / medium / compact 三态：宽屏为左对齐紧凑操作栏和左右分栏；中等宽度为三列两行操作栏和上下分栏；紧凑宽度为两列三行、隐藏位置列与列宽提示。

```text
菜单专项 62 passed
分片全量 782 passed, 8 skipped（586 + 189 + 5 + 2）
```

全量单进程仍会在 WebM 测试遗留 FFmpeg reader 线程后进入 Cocoa 快速对话激活测试时原生 abort；隔离 `test_second_batch.py` 与 macOS 激活文件后，785 个测试全部获得完成结果且无断言失败。
