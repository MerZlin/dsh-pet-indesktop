...
> **历史快照提醒**：本手册对应 2026-09-03 时 perf/stage-1 分支的状态，其中部分
> 数值（尤其性能线）已被后续批次更新——例如「首帧缓存 32MB 预算」在批10-A3 后
> 默认改为 8MB。维护者请以当前代码与 `_plan/current/` 档案为准。
>
> **注**（2026-09-05，批5.3 之后）：共享解码已由**进程内帧扇出（DecodeFanoutHub）**
> 取代，`decode_broker_enabled` 与 `decode_broker.py` 均已移除；下文相关描述仅作存档，
> 请以当前代码为准。
