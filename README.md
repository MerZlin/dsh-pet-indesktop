...
> **当前开发版（main，2026-09-08）**：已合并 PR #73、#76、#79、#80。相比 v4.1.0 发布版，当前主线新增了桌宠内存专项治理（长时运行不再单调上涨）、单进程多窗与共享解码链（3 窗 1 进程 1 解码器）、打字频闪根治、直播捕获气泡与 macOS Dock 隐藏修复、识屏自我识别提示、Harness 实例复用与自启、设置对话框拆分与死代码清理等结构治理；同时固化了一批开发约束与规则（见下节）。发布形态仍为 **onedir 目录打包 + Inno Setup 安装包（`.exe`）+ 便携 zip 绿色版**：安装版与绿色版运行期都不解压、不产生临时缓存，启动快、卸载干净。
...
- **多桌宠围圈聚集互动（默认关闭）**：多只桌宠可被“召唤”到同屏聚在一起，2 只面对面、≥3 只围圈，再同播同一个“一起吃”类动画；支持右键触发和可选的“靠近时自动聚一聚”。依赖碰撞通道，默认关闭时零常驻 timer/线程/解码开销；移动过程播放移动动画走到目标位置，任一只被交互/隐藏/退出即取消整场。
- **聚集动画可选**：设置 → 常规 → 多开新增“聚集动画”选择列表（随机 / 当前角色实有动画）；选择固定动画后，右键「聚集互动」直接播放该动画，自动偶遇优先播它；参与者不足或没有共有动画时菜单置灰并给出明确原因，不再出现“空内容”二级菜单。聚集开关按桌宠各自生效，已有小肥鱼需各自开启（清除后重新生成会跟随主设置）。
- **子肥鱼大小修复**：统一多进程/单进程子槽落种换算，关闭“生小肥鱼继承大小”后新生成的子肥鱼严格使用“小肥鱼大小”；“继承灵动岛”同理。已有旧子鱼不迁移，清除后重新生成即生效。
- **灵动岛精简右键菜单**：在灵动岛胶囊上右键可直接切换“信息槽内容（时间/余额/余额峰谷/自定义短文本）”与“外观（深色/浅色/玻璃）”，选择即时保存；无常驻菜单开销，左键拖拽/单击行为不变。
- **余额查询音效现状说明**：当前版本与上游 dsh-pet 都未实现“查询余额音效”；余额查询只有气泡 + 档位动画。本次不做新增，仅记录结论。
