...
2. **`idle_turn/` 内部再区分待机和转向**：
   - 文件名包含 `待机`、`idle`、`呼吸` → 待机
   - 文件名包含 `转向`、`转身`、`东张西望`、`回头`、`turn` → 转向
   - 如果只有一个文件，则作为待机；
   - 如果有多个文件且无法按关键词区分，第一个作为待机，第二个作为转向。

3. **可选 `manifest.json` 补充/覆盖**：
   - 查找位置：
...
5. 如果某个角色没有可用的“待机”，程序会安全回退到该角色第一个动画，避免启动崩溃。

> 建议：新角色推荐直接使用 `idle_turn/ move/ click/ drag/ random/` 子目录结构，
