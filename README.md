...
>
> **开发中分支（未合并）**：把「键鼠跟随」重做成通用的 **外接启动模式**——主桌宠只负责启动用户自己安装的外部桌宠程序（如 BongoCat），不再 fork / 编译 / 内置任何第三方程序，并提供一整套自定义角色的素材工具链。详见 [模式切换（外接启动模式）](#模式切换外接启动模式) 与 [`docs/EXTERNAL-MODE-2026-09-16.md`](docs/EXTERNAL-MODE-2026-09-16.md)。
>
