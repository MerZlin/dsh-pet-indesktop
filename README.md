...
>
> **开发版 main（未发布，v4.2.1 候选）**：已并入 PR #97（事件汇报概率门 + 事件链路语义对齐 + issue #95：Windows node/pnpm 解析不再写死）、#100 / #102（启动即按配置装配可选服务，issue #99）、#101（边缘探头会话期间禁止位移）、#104 / #105（桥接插件归零外部依赖 + DSH 事件层系统性收尾）、#107（余额多币种盲取首条）、#108（弹射卡顿）、#109（探头/头槌体验 + 气泡分页）、#112（Windows 关机/注销弹 0xc0000142）、#113（灵动岛重做）、#114（碰撞卡顿三连修）、#118 / #125（语音报时 + 台词库轮换 + 包体瘦身）、#120（点击音效粘错误态永久静音）、#121（点击桌宠不再夺走前台焦点，issue #98）、#123（气泡长文本体验 + 菜单/设置页收口）。本机全量套件 2192 passed / 10 skipped，新增内容见下方 [最近修复与变更记录](#最近修复与变更记录)。
>
> **开发中分支 `codex/key-mouse-mode`（未合并）**：把「键鼠跟随」重做成通用的 **外接启动模式**——主桌宠只负责启动用户自己安装的外部桌宠程序（如 BongoCat），不再 fork / 编译 / 内置任何第三方程序，并提供一整套自定义角色的素材工具链。详见 [模式切换（外接启动模式）](#模式切换外接启动模式) 与 [`docs/EXTERNAL-MODE-2026-09-16.md`](docs/EXTERNAL-MODE-2026-09-16.md)。
>
...
### PR #125 / #124（2026-09-16 合并，KagurazakaChizuru）——语音报时 v2：台词库 8 小时整批轮换 + 气泡/语音文本解耦 + 包体瘦身

> #124 因与刚合并的 #123 冲突而无法直接合并，改由 #125 承接落地（内容与 #124 逐字节一致，仅补冲突解决）。合并过程记录见 [`docs/PR-REPORT-VOICE-CHIME-2026-09-15.md`](docs/PR-REPORT-VOICE-CHIME-2026-09-15.md)。

- **台词库整批轮换**：语音报时的候选台词改为按 **8 小时为一个整批**切换（同一批次内不重复抽到同一句），而不是逐句随机；跨批时整批替换，避免短时间内反复听到同一句。
- **气泡文本与语音文本解耦**：同一次报时里，**气泡显示的文本**和**念出来的语音**各自有独立台词来源，可以只用语音、只用气泡，或两者用不同文案。
- **逐事件自定义文案**（承接 #123）与语音报时设置页随之一并落地。
- **包体瘦身**：新增 `scripts/slim_bundle.py`，打包后剔除运行时用不到的冗余文件；`scripts/build_onedir.ps1` 同步接入，另有 `tests/test_bundle_slim.py` 守门。
- 验证：`ruff` 全过、全量套件 2192 passed / 10 skipped、三平台 CI 全绿。

### PR #123（2026-09-16 合并，klxxya）——气泡长文本体验 + 菜单/设置页折叠收口

- **气泡宽度自适应**：短文案保持 248px，长文案放宽至 360px 上限；审批/提问气泡布局不动，窄屏与直播捕获子模式按可用区收敛。
- **长文本源头截断**：联动过程汇报 80 字封顶加「…」；快速对话回答 150 字封顶并提示「全文见聊天窗」（会话历史仍保留全文）。
- **分页体验**：末页孤行控制收紧到 ≤2 行（3+2 → 2+3 重平衡）；末页播完压一拍再回首页，不再硬切。
- **右键菜单**：自定义联动 Agent 收进「自定义联动 Agent」三级子菜单。
- **设置页**：逐事件自定义台词收进独立折叠框并**默认折叠**（恢复此前在改版中丢失的行为），概率门滑块保持常显；搜索命中时两个框都会自动展开。
- **测试债**：拖拽合帧定时器断言改为刷新率感知（高刷屏下原断言恒失败）。

### PR #121（2026-09-16 合并，MerZlin）——点击桌宠不再夺走前台/键盘焦点（issue #98）

- **问题**：点一下桌宠后，全局 Ctrl+C / Ctrl+V「失效」——其实是键盘焦点被桌宠抢走，快捷键发到了桌宠窗口，得重启桌宠才恢复。
- **根因**：桌宠窗口没有 `WS_EX_NOACTIVATE` 扩展样式，真实鼠标点击会让它成为前台窗口并拿走键盘焦点。
- **修复**：`pet/platform_win.py` 增加 `_set_windows_no_activate()`（只改扩展样式，不重建窗口），窗口 `showEvent` 时应用；点击仍能正常触发交互。
- **取证**：仓库里没有任何 `OpenClipboard` / `SetClipboardData` / 剪贴板监听调用，剪贴板在桌宠启停前后都保持可打开——排除「桌宠占用剪贴板」的猜测。定位用 `scripts/diagnose_clipboard.py` 现场取证，跨进程用例 `tests/test_foreground_steal.py`。
- **已知取舍**：Windows 上弹弓的 ESC 取消不再可达（右键取消不受影响）。

### PR #120（2026-09-16 合并，MerZlin）——点击音效变「永久静音」（issue #116 问题二）

- **问题**：点击音效某次开始不出声，之后**再也不出声**，重启才恢复。
- **根因**：`QSoundEffect` 一旦进入 `Error` 状态就**粘住**，此后 `play()` 是静默空操作；旧代码只在缺文件时重建音效对象，粘在错误态的对象永远不会被换掉。
- **修复**：`pet/click_sound.py` 在播放前检查音效状态，处于 `Error` 时丢弃缓存对象并按路径重建；重建后仍是 `Error` 才返回失败。

### PR #118（2026-09-16 合并，KagurazakaChizuru）——新增「语音报时」功能

- 整点/自定义时刻用 edge-tts 语音报时，可选同时显示气泡；可配置音色、语速、音调、音量、自定义时刻表与是否播台词。
- 设置页新增独立分组，配置键（`voice_chime_*`）纳入 reload 白名单；退出收口、停止后作废在飞请求、依赖声明同步加固（后续由 #125 扩展为台词库整批轮换）。

### PR #114（2026-09-13 合并，klxxya）——碰撞卡顿三连修

- **落地预热离线程**：落地瞬间的首帧预热不再卡 GUI 线程。
- **飞行期首帧 pin**：飞行途中固定使用已预热的首帧，避免中途冷解码。
- **岛内容层缓存**：灵动岛内容层改为缓存重建，减少每帧重绘开销（守门用例 `tests/test_island_content_cache.py`、`tests/test_warm_landing_idles.py`）。

### PR #113（2026-09-13 合并，klxxya）——灵动岛重做：事件动效 / 四边停靠 / 快捷卡片

- 灵动岛改为事件驱动的动效呈现，支持四边停靠与快捷卡片；果冻墙改为本进程直连碰撞。
- 池外新增 `pet/dynamic_island.py` 的可见性聚合（`set_pet_visible`）与多窗可见性同步。
...
- [`docs/PR-MERGE-LESSONS-2026-09-12.md`](docs/PR-MERGE-LESSONS-2026-09-12.md)：**合并 PR 前必读**——叠放 PR 的 squash 冲突、行数红线是「组合性质」、时序用例写法三则实测教训。
- [`docs/EXTERNAL-MODE-2026-09-16.md`](docs/EXTERNAL-MODE-2026-09-16.md)：外接启动模式（模式切换）的设计、配置与验收清单。
- [`docs/KEY-MOUSE-MODE-OWN-CHARACTER-2026-09-15.md`](docs/KEY-MOUSE-MODE-OWN-CHARACTER-2026-09-15.md)：自定义键盘模型素材的 GPT 提示词包（配合 `scripts/prepare_bongo_own_character.py` 等工具）。
