...
- **macOS 对话报「网络连接失败：[SSL: CERTIFICATE_VERIFY_FAILED]」**：发布包此前未内置 CA 证书库（macOS 上 Python 默认 CA 路径为空，任何 HTTPS 请求都会校验失败）。现已通过 `--collect-all certifi` 将 CA 证书包（`cacert.pem`）打进发布包；同时在 AI 设置中新增「跳过 SSL 证书验证」选项，用于本地网关 / 自签名证书 / 代理拦截场景（仅建议在可信的内网 / 本地环境关闭校验）。
- **AI 设置「测试连接」按钮**：由占位提示改为真实连通性测试——发送一个最小请求（含 TLS 校验，10 秒超时），结果直接在对话框内显示；并修复了连续多次点击「测试连接」导致进程崩溃退出的问题（后台线程对象生命周期竞态，改用 Python daemon 线程 + 信号回主线程，不再使用 QThread）。
