...
> **当前版本：v4.0.3**（2026-08 紧急修复版，修复点击穿透/桥接安装/中文乱码）。发布形态为 **onedir 目录打包 + Inno Setup 安装包（`.exe`）+ 便携 zip 绿色版**：安装版与绿色版运行期都不解压、不产生临时缓存，启动快、卸载干净。
...
- **开发版（main，尚未发版）**：余额分档动画、DeepSeek 峰谷提示（可自定义文案）、后台音乐自动唱歌、移动动画调整、位置记忆修复、自启残留清理、thinking 专属气泡文案等（PR #29/#30/#31/#32）。
...
- **DeepSeek 峰谷提示（PR #31/#32）**：余额气泡下方显示当前高峰/空闲与下一切换时间；可在设置中选择默认「空闲/高峰」、预设「梁文谷/梁文峰」，或自定义高峰/空闲文本。
