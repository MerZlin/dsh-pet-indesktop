...
程序会按以下方式区分“待机 / 转向 / 移动 / 点击回应 / 拖拽 / 随机动作”：

1. **优先读取角色目录下的 `manifest.json`**（最精确）：
   - 查找位置：
   ...
   - 示例：
     ...
     ```
   - 文件名也可以是去掉 `.webm` 后的动画名。
   - 未在 manifest 中指定的动画会自动进入随机动作池。

2. **如果没有 manifest.json**，则使用内置已知名称 + 文件名关键词自动识别：
   - 待机：包含 `待机`、`idle`、`呼吸`
...
3. 如果某个角色没有可用的“待机”，程序会安全回退到该角色第一个动画，避免启动崩溃。

> 建议：如果你希望 100% 精确控制分类，尤其是自定义动作名和内置关键词差异较大时，
> 在新角色目录中放一个 `manifest.json`。
