...
- **每只独立**（右键某只 → 桌宠设置，改哪只影响哪只）：形象/缩放/透明度/置顶、拖拽物理/弹射/音效、自言自语、省电降帧、AI 对话与识屏、位置。各只设置存于各自的 `config-slot-N.json`。
- **进程级互通**（全窗共用一份，随主配置生效）：托盘与灵动岛、DeepSeek 余额查询、更新检查、待办提醒、共享解码链、单进程多开开关本身。
