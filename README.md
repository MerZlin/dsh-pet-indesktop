...
> **当前稳定版本 v4.2.1（2026-09-23 发布）**：下方保留 v4.2.0 以来的完整历史汇总；当前插件化与工程规范化施工以 [项目规范](SPEC.md) 为准。
>
> v4.2.0 历史基线：自 v4.1.0 以来的功能与修复汇总（PR #57/#64/#65/#68/#70/#71/#72/#73/#76/#79/#80/#82/#85/#86/#87/#90/#91/#93/#94/#96）。
...
.\.venv\Scripts\python scripts/check.py --fast
.\.venv\Scripts\python -m pet
```

- Python 支持范围：3.11–3.13。
- 详细质量门、插件化边界和回滚纪律见 [`SPEC.md`](SPEC.md)。
- 文档唯一入口见 [`docs/INDEX.md`](docs/INDEX.md)；变更事实日志见 [`LOG.md`](LOG.md)。
- `scripts/check.py --ci` 是 CI 与本地完整门禁的统一入口；快速反馈可使用 `scripts/check.py --fast`，跨 Python 版本质量矩阵使用 `scripts/check.py --quality`。完整门禁会合并隔离测试族的覆盖率后再执行 83% 阈值检查。
...
4. **`window.py` 行数预算**：当前预算 `4648` 行（常量 `WINDOW_PY_LINE_BUDGET` 见 `tests/test_architecture.py`，实测 4648 行，2026-09-23 校准）。历次校准理由逐行记在该文件注释里（含 #101/#102 启动装配与探头闸门、#108 弹射飞行守卫、#109 探头旋转/软撞位移、#112 会话结束的 `_closing` 守卫、#182 的实时性改造与上游合入）。预算只随实测校准，**不靠压缩行宽/合并语句硬塞**；确需上调要在 PR 说明理由。
5. **`modern_settings_dialog.py` 行数预算**：当前预算 `2347` 行（常量 `MODERN_SETTINGS_DIALOG_PY_LINE_BUDGET`，实测 2343 行，2026-09-22 随 #179 整域拆分**首次下调**；已拆出 `pet/settings_interaction.py`、`settings_widgets` / `settings_menu_layout_editor` / `chat/ai_settings_page` / `settings_theme_qss`），再往主对话框塞新页面属于红线。
...
### PR 交付证据（三份证据，2026-09-22 起硬要求）

每个 PR 必须随附**三份证据**，缺一即视为未完成，审查时按缺陷提出（出处：[`AGENTS.md`](AGENTS.md) 的 "Delivery evidence discipline"，细则见 [`docs/DEV-HANDOVER.md`](docs/DEV-HANDOVER.md) §8.2/§8.3）：

1. **修改文件说明**：逐文件写「改了什么 + 为什么」，给出增删行数（`git diff --numstat`），新增/删除的文件都要列出。只写「修了 bug」不算说明。
2. **性能分析**：受影响路径的**实测**数字（命令 + 环境 + 样本量），并逐条回答：稳态开销、新增路径的成本与触发频率、有没有新的系统调用/网络/磁盘/线程、内存有无增长。**形容词不算分析**——用「可忽略 / 更快 / 优化了」代替数字视为没写。
3. **实机测试记录**：在本机真实环境（**不是 CI、不是 mock**）跑过什么的记录——命令、真实输出、以及用户可见行为的确认。无法自动验证的能力，必须给出「为什么不能自动」的排查证据（探针结果），不许用沉默代替结论。

落地形式：报告写入 `docs/PR-REPORT-<主题>-<YYYY-MM-DD>.md`（骨架照抄 [`docs/PR-REPORT-TEMPLATE.md`](docs/PR-REPORT-TEMPLATE.md)），并在 [`docs/INDEX.md`](docs/INDEX.md) 的「PR 报告存档」登记一行；PR 描述放**摘要 + 报告链接**，不要把报告全文粘进去。**豁免**：纯文档/文案/依赖版本号这类不改变运行行为的改动，可只保留第 1 条，但必须在 PR 描述里写明豁免理由。机器化校验：`tests/test_pr_report_discipline.py` 对 2026-09-22 之后的报告强制校验三个必备章节（标题级）与索引登记。

## 目录

- [版本亮点（v4.2.1）](#版本亮点v421)
- [v4.2.0 以来的变更（v4.2.1）](#v420-以来的变更v421)
- [开发约束与规则](#开发约束与规则)
- [项目来源与素材声明](#项目来源与素材声明)
- [当前状态](#当前状态)
- [下载与版本选择](#下载与版本选择)
...
v4.2.1 覆盖 v4.2.0 之后合并的 **57 个 PR / 192 个提交**，其中有 5 位外部贡献者各带了一整批（klxxya 26 个、Anguis-L 4 个、KagurazakaChizuru 3 个、Daliuq 2 个、lscatfish 2 个，逐人致谢见发布说明）。逐项清单见下方 [v4.2.0 以来的变更（v4.2.1）](#v420-以来的变更v421)，完整发布说明见 [`docs/RELEASE-v4.2.1.md`](docs/RELEASE-v4.2.1.md)。

**设置与菜单（本版最直观的变化）**
- **「互动」域改页内标签**（设置 → 互动：「点击与音效」/「自言自语」），侧栏 9 个域与组名不变；设置搜索命中非当前标签的行会**自动切过去**。
- **音乐播放器路径进设置页**（设置 → 桌宠 → 音乐关联：两行路径 + 「自动检测」按钮），不必再手改配置文件。
- 气泡文字大小 50–300% 可调、**Harness 服务可重启/停止**、设置窗内可直接**裁切聊天背景取景**、「多开」开关从设置页收起（配置键原样保留）。

**音乐与歌词**
- 右键新增 **音乐 → 歌词对齐**（回到开头 / 上一句 / 下一句 / 后退·前进 5 秒）：网易云**不上报 SMTC 播放进度**，歌词只能按本地时钟估算，快进或半途起播后用它对一句即可；报进度的播放器自动置灰。
- 新增**「上一首」**、修「关掉再打开歌词功能后歌词永久不动」、`music_player_paths` 支持。
- **开着 VPN / 系统代理时歌词取不到词已修**（歌词请求改直连，实测同一首歌 9.01s / 0 行 → **0.88s / 53 行**）；网易云「音乐 → 歌词对齐」不再被误置灰。逐功能影响面见 [`docs/NETWORK-PROXY-AND-VPN-2026-09-22.md`](docs/NETWORK-PROXY-AND-VPN-2026-09-22.md)。

**台词朗读、语音与内容**
- **点击台词朗读默认开**（`self_talk_speak_enabled`）：点击桌宠弹出的自言自语会用语音念出来；可选**本机语音预缓存**（CosyVoice `127.0.0.1:9880`，默认关）。
- **配图概率可设**（`self_talk_image_chance`，默认 30），修掉「图片目录一多就几乎轮不到文本」（旧行为实测出图 82.8%）。
- **语音报时** `voice_chime`（六种调度模式）与**节日提醒** `festival_reminder`（46 个日子 / 314 条文案）；edge 音色被微软下架时**自动兜底 + 重试**，0 字节缓存不再算命中。

**桌面玩法、碰撞与灵动岛**
- **灵动岛重做**（事件动效 / 四边停靠 / 快捷卡片）；岛碰撞改**同步硬墙**，杜绝穿透与区内反复反弹。
- **鱼-鱼碰撞体改用身体框**、抛掷中只挡不弹；**多进程子宠补挂远端硬墙**（修多开时子肥鱼穿岛）。
- 探头/头槌体验三连修、边缘探头会话期禁位移、**移动-动画同步与朝向规则重写**（消除「溜冰感」）。

**流畅度、内存与包体**
- 走路位置交付 **28.6Hz → ~160Hz @170Hz**；webm 冷路径首帧不再在 GUI 线程同步解码，碰撞压力下 >50ms 卡顿 **133 次 → 3 次**；音效包候选解析进程级缓存（**0.43ms/次**）。
- 绿色版 zip **176.2 MB → 146.5 MB**；单进程多开 + 同角色共享解码链、省电模式（闲置 30s 半帧率，解码 CPU −54.6%）。

**工程与质量**
- **PR 交付证据纪律**落地并由 `tests/test_pr_report_discipline.py` 机器化校验（见上文「开发约束与规则」）；构建新增 **Qt 绑定排他**（否则 PyInstaller 在收集阶段直接中止）与**产物 TTS 自检**。
- 设置页结构治理：整域抽 `pet/settings_interaction.py`，`modern_settings_dialog.py` **2441 → 2347 行**，行数预算首次因拆分下调。
- 测试：本机全量 **2913 passed / 11 skipped / 0 failed**，三平台 GitHub Actions 全绿。

<details>
<summary><b>上一个版本亮点（v4.2.0）</b></summary>
...
`v4.2.0`（2026-09-10 发布）之后，`main` 上又积累了 **54 个已合并 PR、约 175 个提交**（另有若干直推修复），**全部尚未发布**——当前正式版本仍是 v4.2.0。本节是这批变更的总览；带根因与验证细节的逐条记录见下方 [最近修复与变更记录](#最近修复与变更记录)；面向发布的完整说明（含 **发布前测试清单** 与 **视频预演脚本**）见 [`docs/RELEASE-v4.2.1.md`](docs/RELEASE-v4.2.1.md)。

### 新增功能

**桌宠玩法与外观**
- **灵动岛重做（#113）**：事件动效、四边停靠、快捷卡片；果冻墙改本进程直连碰撞。
- **气泡文字大小可调（#153）**：新配置键 `bubble_text_scale`（50–300%，默认 100，设置 → 外观 → 桌宠显示）。列宽、换行预算、label 尺寸与字号**共用同一系数**整体等比放大，标准气泡与呼吸气泡都生效；默认 100% 时与旧版逐像素一致。
- **聊天背景裁切取景（#163 / #167 / #169 / #170）**：主设置窗新增取景入口，编辑结果随保存落盘；裁切选区纵横比按编辑风格取值；入口标注当前风格、非填充裁剪时隐藏（渲染链恢复见 #163）。
- **移动-动画同步与朝向规则重写（#157）**：位移锁定步态整圈、位置解码帧驱动，消除「溜冰感」；朝向永远由目标决定，不再被随机数翻转。

**设置与菜单**
- **「互动」域设置页分页（#179）**：设置 → 互动 由 18 行 / 3 组平铺（当时最长的「无折叠」域）改为两个页内标签——**「点击与音效」**（输入 + 点击反馈）与**「自言自语」**（周期气泡的台词 / 节奏 / 配图）；侧栏 9 个域与组名 `输入` / `点击反馈` / `自言自语` 属对外契约，一律不变。设置搜索命中非当前标签的行时**自动切到那个标签**（搜「配图概率」→ 切到「自言自语」）。`modern_settings_dialog.py` **2441 → 2347 行**（首次因拆分下调行数预算），整域搬进新模块 `pet/settings_interaction.py`。
- **音乐播放器路径进设置页（#177）**：设置 → 桌宠 → 音乐关联 新增两行路径（网易云音乐 / QQ音乐）+「自动检测」按钮，不必再手改 `config.json`。
...
- **点击台词朗读 `self_talk_speak_enabled`（#176，默认开）**：点击桌宠弹出的自言自语直接用语音报时的音色念出来（与报时共用音频通道、不叠音）；不想要就关掉这一行。同时把**「点击触发自言自语」的显隐与朗读与「气泡自言自语」总开关解耦**——显示与朗读只认 `click_show_self_talk`（此前关掉定时气泡会连点击台词一起消失）。
- **台词自动预缓存 `self_talk_voice_precache_enabled`（#176，默认关）**：保存设置后与每次启动 9 秒后，在后台把「点击台词 + 点击动画绑定台词」合成为本机语音文件（`<配置目录>/self_talk_voice/<文本摘要>.wav`）——命中即本地播放，**点击秒出、断网也能说**；幂等补齐（只补没有音频的句子）、单飞不叠线程、0 字节残文件当缺失重合成、服务不可用整轮跳过只记一条日志。**需要本机 CosyVoice 语音服务**（`http://127.0.0.1:9880` 的 `/health` 返回 `ok`；本机合成一句约 20–60 秒，所以只在后台做）；没装就保持关闭，点击仍走在线合成、不会变哑。
- **自言自语「配图概率」`self_talk_image_chance`（#176，默认 30）**：改为**先掷骰决定出图还是出文本**，再在对应池内等权选。此前是「文本 + 图片拼成一个列表等权随机」，图片目录里几十张图时实际出图率可达 80%+、文本几乎轮不到（实测 24 图 + 5 句 → **82.8%**）。`0` = 只出文本、`100` = 只出图；想恢复旧观感填 `图数 ÷ (图数 + 文本数)`。只有一边有内容时不掷骰子（避免「点一下什么都不显示」）。**改完需重启桌宠生效。**
...
- **Harness 服务可重启 / 停止（#153）**：菜单项升级为子菜单（启动并打开页面 / 重启服务 / 停止服务）；停止时按端口反查进程并核验命令行确为 `dsh web`，端口被非 dsh 进程占用或反查不到属主时一律**不终止**，只如实报告。
...
- **音乐歌词跟随快进（#175）**：实测**网易云音乐完全不上报播放进度**（SMTC `position` / `end_time` 恒为 `0.0`，播放中、暂停、切歌、快进四种状态采样结果一致），歌词只能按本地时钟估算，于是快进或从中途开始播放必然错位。新增**右键菜单「音乐 → 歌词对齐」**（回到开头 / 上一句 / 下一句 / 后退 5 秒 / 前进 5 秒；报进度的播放器会自动置灰并说明理由），并把"开启功能时歌已在播"从**静默跳过**（连歌名都不显示）改为**先亮歌名 + 提示一次可对齐**；同时修「向后拖到第一句歌词之前仍显示旧句」与**多会话下跳到浏览器里那首歌**（会话选取改为粘住当前播放器）。
...
**语音与提醒**
- **edge 音色被下架后「配了音色却没声音」（#178）**：微软下架了 10 款 `zh-CN` 音色（晓辰 / 晓涵 / 晓梦 / 晓墨 / 晓秋 / 晓睿 / 晓双 / 晓萱 / 晓颜 / 晓悠），配置指向它们时 `edge-tts` 抛 `NoAudioReceived: No audio was received. Please verify that your parameters are correct.`，用户只看到一句「语音合成失败（NoAudioReceived: …）」；更隐蔽的是缓存目录里留下的 **0 字节 mp3 会被当成命中**——播出一声静音且**永远不再重合成**。现在：配置音色不在可用表里 → **自动改用默认音色「晓晓」**并提示一次（`音色「晓涵」已不在微软在线音色表里（多半被下架了），本次改用默认音色「晓晓」；可在 设置 → 语音 → 语音报时 → 音色 里换一个可用的。`，每进程只提示一次）；每个候选音色最多重试 **2 次**（失败后间隔 **1.5 s**）再退默认音色重试；**0 字节缓存判为未命中**并清理半截产物（服务端「成功但空文件」也算失败）；内置音色清单按微软在线表重建为 **36 款（中文 14 + 英文 22）**，设置页里不在清单的配置值显示为「**不在清单里：<值>**」。实机复核：用已下架音色触发 → 自动改用默认音色并产出 22 752 字节音频；`list_voices()` 返回 322 款。
- **语音报时 / 提醒链路加固**（#159 / #160，逐条清单见下方「工程、测试与文档」）：提醒队列死锁、看门狗误杀新连接、菜单销毁后访问已删对象、POSIX 残留子进程；以及卡住检测定时剪枝、报时 busy 门改排队补播、预热分类口径、`quick_chat` 气泡形状、连接测试线程安全。
...
- **交付证据纪律（#180）**：每个 PR 必须交付「修改文件说明 / 性能分析 / 实机运行记录」三份证据，模板 `docs/PR-REPORT-TEMPLATE.md`，由 `tests/test_pr_report_discipline.py` 机器化校验（校验模板存在、`AGENTS.md` 与 `DEV-HANDOVER.md` 仍写明要求、生效日之后的报告标题级含三章节且在 `docs/INDEX.md` 登记）；**2026-09-22 起生效，历史报告豁免**。
- **构建 Qt 绑定排他 + 产物 TTS 自检（#180）**：打包机上装了 PyQt5 时 PyInstaller 会在收集阶段**直接中止**（`ERROR: Aborting build process due to attempt to collect multiple Qt bindings packages…`），而 `edge-tts` 正是在收集阶段被打断的——新增 20 个 `--exclude-module`（其余 Qt 绑定 + 把它们拖进来的 matplotlib / IPython 栈；原有排除一条不减）。另新增 `scripts/verify_bundle_tts.py`：在打包机算出 `import edge_tts` 的导入闭包，与产物 exe 的内嵌 PYZ / `_internal` 清单比对，缺一即中止构建（实测真产物 25 个必需模块全在、`exit=0`；修复前曾因 `.pth` 启动期注入模块与运行时合成模块**假红**，已修并加 7 条回归用例）。
- **角色头部框数据（#180）**：`assets/characters/shenshen/videos/manifest.json` 新增 `head_box`，`pet/catalog.py` 新增 `character_head_box()`（未声明时按身体框上缘取 45% 兜底）——为「边缘探头只露头」铺路；**当前零调用方、本版尚未接线**。
- **设置页结构治理（#179）**：整域抽 `pet/settings_interaction.py`，`modern_settings_dialog.py` **2441 → 2347 行**、行数预算首次因拆分下调；「自动化与联动」（94 行）仍是最长域，按同法分页是下一步。
- **edge-tts 音色事故档案 + 回归（#178）**：新增 `docs/ISSUE-EDGE-TTS-VOICE-DEPRECATION-2026-09-22.md`（排查链与残留风险）与 13 条离线回归用例 `tests/test_voice_chime_edge_voice.py`（重试、退默认音色、空音频算失败、非空缓存判定、提示只弹一次）。
...
- **v4.2.1（2026-09-23 发布）**：v4.2.0 之后并入 57 个已合并 PR、192 个提交（PR #97–#183）——**新功能**：灵动岛重做（#113）、语音报时 `voice_chime`（#118 / #125，默认关闭）、节日提醒 `festival_reminder`（#127）、事件汇报概率门（#97）、气泡文字大小可调（#153）、Harness 服务重启/停止（#153）、聊天背景裁切取景（#163 / #167 / #169 / #170）、音乐歌词显示与「上一首」（#128 / #129 / #135）、**歌词对齐（#175）**、**点击台词朗读 + 本机语音预缓存 + 配图概率（#176）**、**音乐播放器路径进设置页（#177）**、**「互动」域设置页分页（#179）**；**关键修复**：Windows 关机/注销弹 `0xc0000142`（#112，issue #111）、桥接插件归零外部依赖（#104）、DSH 事件层系统性收尾（#105）、启动即按配置装配可选服务（#100 / #102，issue #99）、单进程多开下主动识屏永不触发（#145）、点击桌宠夺走焦点导致全局复制粘贴失效（#121，issue #98）、Linux 屏幕边缘无法贴边（#137）、岛碰撞改同步硬墙（#173）、**edge 音色被下架后自动兜底 + 0 字节缓存不再算命中（#178）**、**构建不再被 PyInstaller 的 Qt 绑定互斥中止（#180）**；另有 `#136` / `#131`（歌词显示审计 + 节日动画/生日）经 `#143` 回退、`#164` 经 `#166` 回滚。逐项清单见 [v4.2.0 以来的变更（v4.2.1）](#v420-以来的变更v421) 与下方 [最近修复与变更记录](#最近修复与变更记录)；**发布说明、验收清单与视频预演脚本见 [`docs/RELEASE-v4.2.1.md`](docs/RELEASE-v4.2.1.md)**。
...
建议使用 Python 3.11–3.13（与当前 CI 和开发质量门一致），并在项目根目录执行：
...
> 三者与「鼠标穿透」的区别：锁定/SHIFT 只是禁止拖动，点击互动（点头、音效、彩蛋）照常；鼠标穿透是桌宠完全不接收鼠标事件（点击会落到下层窗口），需要从托盘菜单关闭（或到「桌宠设置 → 互动」里改）。开启「鼠标穿透」时桌宠会气泡提示恢复位置。
...
- **修复**：走路帧间补点（`movement.move_anim_tick`，位置交付 **28.6Hz → ~160Hz @170Hz**）；webm 冷路径首帧不再 GUI 同步解码、抛掷低速冷切走 warm 闸门、meta 探测后台化；抛掷飞行期动画加速（`flight_anim_speed`）；GUI 冻结采样器；子宠进程**补挂远端硬墙** + 岛几何经碰撞 IPC 发布为静态成员（2s 心跳、变化合流 ≤10Hz）+ `island_local_owned` 双重结算守卫；音效包候选解析改进程级缓存（**0.43ms/次**命中）；设置页隐藏「多开」开关（拓扑收口 Phase A，配置键原样保留回写，存量用户不丢值）。
- **验证**：新增 `tests/test_island_remote_wall.py` / `test_move_sync.py` / `test_first_frame_no_gui_decode.py` 等 10 个测试文件；受影响时序族（collision / island / webm / move_sync / throw / spawn / first_frame / meta / click_sound）高负载复跑 **3 遍全绿**；实机走路 A/B 300 秒、三开互撞 5 分钟、多进程岛墙场景；作者本机全量 **3009 passed / 12 skipped**。完整证据见 [`docs/PR-REPORT-PERF-ISLAND-CONSOLIDATED-2026-09-23.md`](docs/PR-REPORT-PERF-ISLAND-CONSOLIDATED-2026-09-23.md)。
- **诚实声明**：开发期"零拷贝取帧"的回退保留（后续取证表明 2026-09-22 部署版崩溃的真正机理是**绘制重入**——同一设备双 `QPainter`，与零拷贝无因果），该崩溃案**未结案**，跟踪于 `.scratch/single-overlay-window/HANDOFF.md`。

### PR #181（2026-09-22 合并，MerZlin）——歌词取词被系统代理拖死 + 网易云「歌词对齐」被误关
...
### PR #180（2026-09-22 合并，MerZlin）——交付证据纪律 + 构建 Qt 排他 + 产物 TTS 自检 + 角色头部框

- **问题**：2026-09-22 在本机构建 onedir 产物时，PyInstaller 在**收集阶段直接中止**——`ERROR: Aborting build process due to attempt to collect multiple Qt bindings packages: attempting to run hook for 'PyQt5', while hook for 'PySide6' has already been run!`（打包机 site-packages 里装着 PyQt5 148 MB、matplotlib 27 MB、IPython 7 MB…）。而 `edge-tts` 恰恰是在收集阶段被打断的 → 语音**永远**打不进包。另有一个**静默**失败面：打包机没装 edge-tts 时 `--collect-all edge_tts` 只打印一行 WARNING（`skipping data collection for module 'edge_tts' as it is not a package`）就继续构建，产物缺 TTS——而打包版用户没有 pip 可装。
- **修复**：`scripts/build_onedir.ps1` 新增 20 个 `--exclude-module`（其余 Qt 绑定 `PyQt5`/`PyQt6`/`PySide2` + 把绑定拖进来的上游链 matplotlib 家族、IPython/ipykernel/jupyter/zmq/nbformat），**原有排除清单一条不减**；新增 `scripts/verify_bundle_tts.py`，在打包机算出 `import edge_tts` 的导入闭包并与产物 exe 的内嵌 PYZ / `_internal` 清单比对，缺一即中止构建。
- **验证**：Qt 互斥用最小复现钉死（`import PySide6.QtCore` + `import PyQt5.QtCore` → 原文报错、`exit=1`；PyInstaller 报错里给出的处置办法正是 `--exclude`）。产物自检实测：chat 变体 `[tts] bundle TTS stack OK (25 modules required, 2178 frozen)`、`exit=0`、0.605 s；无 chat 变体 2068 frozen / 0.464 s；另现造一个真没 TTS 栈的产物仍报 **25 个必需模块全缺**（`exit=4`）——判定强度没降。**自检脚本自身踩到一次假红**：闭包原先按「`import edge_tts` 后 `sys.modules` 里所有非标准库模块」算，于是 `.pth` 启动期注入的 `google`/`mpl_toolkits`/`pywin32_bootstrap`、Cython 运行时现造的 `cython_runtime`、pkgutil 空壳 `backports` 全被算成「必须有」，真产物被判 FAIL；改为「导入前后做差 + 只认磁盘上有真实源码的模块」并加 7 条回归用例（改前 `5 failed, 2 passed` → 改后 `7 passed`）。
- **同时落地**：交付证据纪律（`AGENTS.md` + `docs/DEV-HANDOVER.md` §8.2/8.3/8.4 + `docs/PR-REPORT-TEMPLATE.md` + `tests/test_pr_report_discipline.py`；2026-09-22 起对报告强制校验三个必备章节 + 索引登记）、`pet/catalog.py character_head_box()` 与 shenshen 的 `head_box`（**零调用方**，探头锚点接线未做）、主动识屏 v4.2.0 的 issue 草稿 `docs/issue-draft-主动识屏v420.md`。体量：11 文件、+781 / −7。

### PR #179（2026-09-22 合并，MerZlin）——「互动」域设置页分页 + 整域抽 `settings_interaction.py`

- **问题**：互动域 18 行 / 3 组**平铺零折叠**，是本仓库最长的「无折叠」域；主人反馈「设置界面有点乱，单页面太多东西」。
- **修复**：改为页内任务标签「点击与音效」「自言自语」；侧栏 9 个域与组名 `输入` / `点击反馈` / `自言自语` 属于对外契约，一律不变；设置搜索命中非当前标签的行时**自动切标签**；整域搬进 `pet/settings_interaction.py`（167 行）。
- **验证**：6 条聚焦用例 + 既有硬编码用例（组名与顺序、显隐联动、section 标题集合、点击音效行位置）全绿；受影响 9 文件 298 passed；全量 2793 passed / 11 skipped。行数：`modern_settings_dialog.py` **2441 → 2347**、预算同步下调（首次因拆分下调）；三档宽度截图 `docs/screenshots/settings-interaction-tabs-2026-09-22/`。非当前标签只是不可见不销毁，`objectName` 仍是 `settingRow_<键>`，保存链一字未改。

### PR #178（2026-09-22 合并，KagurazakaChizuru）——edge 音色下架兜底 + 连发重试 + 0 字节缓存不算命中

- **问题**（用户报告）：语音报时/朗读「配了音色却没声音」。两条独立根因：① 微软**下架了 10 款 `zh-CN` 音色**（晓辰 / 晓涵 / 晓梦 / 晓墨 / 晓秋 / 晓睿 / 晓双 / 晓萱 / 晓颜 / 晓悠），配置指向它们时 `edge-tts` 抛 `NoAudioReceived: No audio was received…`，用户只看到「语音合成失败（NoAudioReceived: …）」；② 连发时偶发服务端返回**空音频**，而缓存里留下的 **0 字节 mp3 会被当成命中**——播出一声静音且永远不再重合成。
- **修复**：配置音色不在可用表里 → 自动改用默认音色「晓晓」并提示一次（`音色「晓涵」已不在微软在线音色表里（多半被下架了），本次改用默认音色「晓晓」；可在 设置 → 语音 → 语音报时 → 音色 里换一个可用的。`）；每个候选音色最多重试 **2 次**（失败间隔 **1.5 s**）再退默认音色重试；**0 字节缓存判为未命中**并清理半截产物；内置音色清单按微软在线表重建为 **36 款（中文 14 + 英文 22）**；设置页里不在清单的值显示为「不在清单里：<值>」。可用音色表来自在线接口、缓存 **6 小时**，只在后台合成线程联网，失败沿用旧表。
- **验证**：新增 13 条离线回归用例 `tests/test_voice_chime_edge_voice.py`（重试成功、退默认音色、空音频算失败且不留文件、非空缓存判定、提示只弹一次、`edge-tts` 缺失仍走既有降级）；实机复核：已下架音色触发 → 自动改用默认音色并产出 **22 752 字节**音频；`list_voices()` 返回 **322 款**；事故档案 `docs/ISSUE-EDGE-TTS-VOICE-DEPRECATION-2026-09-22.md`（已登记进 `docs/INDEX.md`）。**无新增配置键**。

### PR #177（2026-09-22 合并，MerZlin）——音乐播放器路径进设置页 + 后台自动检测

- **问题**：全仓**没有任何 UI 能写** `music_player_paths`；自动搜索（深度 ≤3 的浅扫）扫不到时右键菜单项置灰，提示语是「找不到 X，**可在配置文件中手动指定路径**」——等于要求用户手改 JSON。
- **修复**：设置 → 桌宠 → 音乐关联 新增两行路径（网易云音乐 / QQ音乐）+「自动检测」按钮（后台按常见目录名搜、**20 秒**超时、**300 ms** 轮询，结束后弹「自动检测播放器」结果框）；菜单提示改指设置页；**路径真的变了才** `clear_cache()`（否则右键菜单下次要重扫盘 4–5 秒）。
- **验证**：真机（本机未装播放器）点「自动检测」**4.17–4.77 秒**收敛、GUI 全程不卡；用 `C:\Windows\System32\notepad.exe` 代替播放器验证「写盘 → 菜单消费」链路（`music_player_paths` 落盘、菜单项由灰变亮、tooltip 不再报「找不到」）；缺陷注入证明用例承重（去掉保存时的 `clear_cache()` → 2 条变红）；设置页宽度探针 1100 / 720 px 无溢出，截图 `docs/screenshots/music-player-paths-2026-09-22/`。体量：19 文件、+568 / −9。稳态零新增定时器/线程（不点按钮则 `QTimer` 从不启动）。

### PR #176（2026-09-22 合并，MerZlin）——点击台词朗读 + 本机语音预缓存 + 配图概率

- **问题**：点击桌宠弹出的自言自语**没有语音**；而且「点击触发自言自语」的显隐与朗读被「气泡自言自语」总开关裹住——关掉定时气泡会连点击台词一起消失。另有主人反馈「点击老是冒图片」：文本与图片拼成一个列表**等权随机**，24 张图 + 5 句文本时实际出图率 **82.8%**，文本几乎轮不到。
- **修复**：① 新增 `self_talk_speak_enabled`（点击台词朗读，**默认开**）与 `self_talk_voice_precache_enabled`（后台把点击台词与点击动画绑定台词合成为本机语音文件，**默认关**，需本机 CosyVoice `http://127.0.0.1:9880`）；② 显示与朗读只认 `click_show_self_talk`，与周期气泡**解耦**；③ 新增 `self_talk_image_chance`（**默认 30**）：先掷骰决定出图还是出文本，再在对应池内等权选（`0` 只出文本、`100` 只出图；只有一边有内容时不掷骰子）。
- **验证**：真机点击 A/B（真实 `PostMessage` 点击窗口中心 + 读日志计数，隔离 `APPDATA`）：`chance=0` → 6 次点击 / 4 条朗读 / 6 条音效；`chance=100` → 8 次点击 / **0** / 8。预缓存实测四行日志文案（`已开始后台预缓存台词语音…` / `台词语音预缓存完成：[声线] 台词 -> 文件名` / `播放预缓存台词 xxx.wav（本地，不走网络）` / `台词语音预缓存跳过：本地 TTS 服务不可用`）；本机**无** CosyVoice 时验证「整轮跳过 + 只记一条 info + 不抛异常」。聚焦 264 passed、全量 2774 passed / 11 skipped；三档宽度截图 `docs/screenshots/self-talk-2026-09-22/`。**配图概率默认 30% 是有意的行为变更**，回滚口径：把它填成 `图数 ÷ (图数 + 文本数)`。**改完需重启桌宠生效**（行为在窗口启动时读一次）。

### PR #175（2026-09-22 合并，MerZlin）——歌词跟随快进/半途起播（手动「歌词对齐」）

- **问题**（用户报告）：快进、或从歌曲中途开始播放时歌词必然错位。实测**网易云音乐完全不上报播放进度**（SMTC `position` / `end_time` 恒为 `0.0`，播放中 / 暂停 / 切歌 / 快进四种状态采样结果一致），歌词只能按本地时钟估算；另修「向后拖到第一句歌词之前仍显示旧句」与多会话时跳到浏览器里在播的那首歌。
- **修复**：右键菜单新增「**音乐 → 歌词对齐**」（回到开头 / 上一句 / 下一句 / 后退 5 秒 / 前进 5 秒；报进度的播放器自动置灰并说明理由）；把「开启功能时歌已经在播」从**静默跳过**（连歌名都不显示）改为**先亮歌名 + 提示一次可对齐**；会话选取改为粘住当前播放器。
- **验证**：新增 `tests/test_now_playing_session.py`（224 行）与 `tests/test_music_lyric.py` 扩展；性能实测脚本 `scripts/bench_music_lyric_align.py`；完整证据（含网易云「不上报进度」的实机复现记录）见 `docs/PR-REPORT-music-lyric-align-2026-09-22.md`。

### PR #163 / #169（2026-09-20 合并，MerZlin）——聊天背景取景渲染恢复与入口语义修正

- **问题**：自定义裁切与主题 focus 在渲染链上失效（取景改了画面不跟着变）；取景入口在非填充裁剪风格下仍显示、且不标注当前风格。
- **修复**：#163 恢复取景渲染；#169 入口标注当前风格并在非填充裁剪时隐藏；#170 裁切选区纵横比按编辑风格取值。
- **验证**：设置窗内编辑保存 → 聊天窗即时生效；`tests/test_desktop_pet_features.py` 相关用例同步更新。

### PR #153（2026-09-19 合并，MerZlin）——气泡文字大小可调 + Harness 启停 + 会话底线修复
