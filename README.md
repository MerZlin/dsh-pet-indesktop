...
- 右键菜单 →「网络信息」打开面板：公网 IP、IP 属地（直接判断**是否在中国**）、外网连通性
  （按当前代理设置实测 Google/GitHub 能否访问，附逐目标耗时）。
- 查询走后台线程（实测属地 400~500 ms、连通性 3.5~6 秒），打开即查、可点「重新检测」。
  面板底部注明会联网、IP 将发送至第三方服务。
- 灵动岛信息槽可显示**网络延迟 + 上下行网速**（`info_mode = "network"`，在「设置 → 灵动岛」
  选「网络延迟/网速」）：三个指标各有独立开关。
- **流量说明**：延迟靠 UDP 往返探测（每次约 166 字节，默认 1 秒一跳，可在配置里调
  `network_probe_interval_seconds`）；网速是读网卡计数器，**零额外流量**；
  IP 属地仅在打开面板时查询。停靠收成细条时不探测。
- 总开关 `network_info_enabled`：关闭后菜单项置灰并说明原因。
...
├── network_status.py         # 网络探测（本机 IP/延迟/网速/IP 属地/外网连通性；零 Qt 纯逻辑层）
├── network_info_panel.py     # 网络信息面板（右键菜单「网络信息」打开；查询走后台线程）
