    ...
    def run():
        ...
        while time.monotonic() < deadline:
            ...
            if files:
                ...
                response_path.write_text(json.dumps({"id": request["id"], **response}), encoding="utf-8")
                return
            time.sleep(0.02)
...
def test_request_writes_context_and_parses_success(bridge_dir):
    ...
    ok, detail = dsh_control.request("replan", "sess-1", goal="修好登录", context="风险理由：W6 同类重复", alert_id="exploration-control:sess-1", timeout=5.0)
    thread.join(5.0)
    assert ok is True
...
def test_request_returns_bridge_error_code_on_rejection(bridge_dir):
    captured, thread = _fake_bridge(bridge_dir, {"ok": False, "phase": "not-found", "error": "session-not-found"})
